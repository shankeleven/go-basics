...
*/
	...
	*/
	...
	// think why it works , because the goroutines inside the functions are running concurrently
	...
	can't be cacelled by its children either
	cancel() works exactly like the done channel
	Golang is highly concurrent. When you spin up goroutines, they often represent operations that may:
	Have timeouts (like HTTP requests or DB queries).
	Be cancelled (e.g., a user closes a browser tab).
	Share metadata across multiple services (e.g., tracing IDs).
	this is what context.Context is for.

	Best Practices:
	Do not store context in a struct—pass it explicitly.
	Do not pass nil Context—use context.TODO() or context.Background().
	Use defer cancel() immediately after context creation (except for context.Background() or TODO()).
	Don’t use context.Value for optional parameters—only for cross-cutting concerns like auth, tracing, etc.
	Always respect the Done() channel in your goroutines or functions.


*/

	type mock_Context interface { // named to avoid confusion with the context package
    Deadline() (deadline time.Time, ok bool) //Returns the time when the context will be canceled, if any.
    Done() <-chan struct{} // Returns a channel that is closed when the context is canceled or times out.
    Err() error // Returns an error indicating why the context was canceled, or nil if it hasn't been canceled.
    Value(key any) any // Returns a value associated with the context for a given key, or nil if not found.
}


// gd on the context and read the file , it is really good
// main ones are withTimeout, WithCancel, WithValue and WithDeadline
 // ctx,cancel := context.WithTimeout(context.Background(), 2*time.Second) // create a context with a timeout of 2 seconds
 // this is also cacellable using the cancel function but there is one added way to cancel it i.e. a timeout
	...
	*/
	...
	wg.Wait()
	fmt.Println("moved past the waitgroup")
	...
	*/
	...
	for i := 0; i < num_jobs; i++ {
		for result := range results { // earlier i wrote the close(Result) after this loop but the range results would be blocking until the channel is closed
			fmt.Println("Result:", result)
		}
	}
